            ...
            <div class="mx-auto max-w-4xl">
                <div class="">
                    ...
                    <div class="flex items-center pt-5 md:pt-10">
                        ...
                        <div class="pl-5">
                            <span class="font-body block text-xl font-semibold text-dark dark:text-white">
                                By: Guido Famula
                            </span>
                            <span class="font-body text-grey-30 block pt-1 text-xl text-secondary dark:text-slate-200">
                                <i class="fas fa-regular fa-calendar-check text-primary dark:text-slate-400"></i>
                        ...
                        <div
                            ...
                            @foreach ($categories as $category)
                                <a href="{{ route('landing.post-category', ['slug' => $category->slug]) }}"
                                    class="max-sm:text-center font-body hover:bg-grey-20 rounded-xl bg-primary py-1 font-bold text-white transition duration-300 ease-in-out dark:hover:bg-secondary md:mr-1 md:px-4">
                                    {{ $category->title }}
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="prose max-w-none pt-8">
                    ...
                    <h4 class="mb-3 mt-10 text-lg font-bold uppercase text-primary">
                        <span class="text-dark dark:text-slate-400">Tag: </span>
                        {{ $post->title }}
                    </h4>
                    <div class="flex pt-3">
                        @foreach ($tags as $tag)
                            <a href="{{ route('landing.post-tag', ['slug' => $tag->slug]) }}"
                                class="font-body hover:bg-grey-20 mr-1 rounded-xl bg-primary px-4 py-1 font-bold text-white transition duration-300 ease-in-out dark:hover:bg-secondary">
                                #{{ $tag->title }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <div class="border-lila mt-10 flex justify-between border-t py-12 dark:border-slate-500">
                        @if ($previous)
                            <a href="{{ route('landing.post-detail', ['slug' => $previous->slug]) }}"
                                class="flex items-center">
                                <span
                                    class="font-body block pr-2 font-bold uppercase text-primary dark:text-slate-200 sm:text-xs md:pr-5 lg:text-lg">
                                    <i class="fas fa-solid fa-chevron-left text-2xl text-primary dark:text-slate-200"></i>
                                    Sebelumnya
                                    <p
                                        class="transition duration-300 ease-in-out hover:text-dark hover:opacity-80 dark:text-slate-200 dark:hover:text-primary">
                                        {{ Str::limit($previous->title, 25) }}</p>
                                </span>
                            </a>
                        @endif
                        @if ($next)
                            <a href="{{ route('landing.post-detail', ['slug' => $next->slug]) }}"
                                class="flex items-center">
                                <span
                                    class="font-body block pl-2 text-right font-bold uppercase text-primary dark:text-slate-200 sm:text-xs md:pl-5 lg:text-lg">
                                    Selanjutnya
                                    <i class="fas fa-solid fa-chevron-right text-2xl text-primary dark:text-slate-200"></i>
                                    <p
                                        class="transition duration-300 ease-in-out hover:text-dark hover:opacity-80 dark:text-slate-200 dark:hover:text-primary">
                    ...
                    <div
                        class="border-lila flex flex-col items-center border-t py-12 pt-12 dark:border-slate-500 md:flex-row md:items-start xl:pb-20">
                        <div class="w-3/4 sm:w-2/5 lg:w-1/4 xl:w-1/5">
                            <img src="{{ asset('assets/images/guido-profil-2.png') }}"
                                class="rounded-full border-t-2 shadow dark:border-slate-500" alt="Guido Famula" />
                        </div>
                        <div class="ml-0 text-center md:ml-10 md:w-5/6 md:text-left">
                            <h3 class="font-body pt-10 text-2xl font-bold text-secondary md:pt-0">
                                <a class="transition duration-300 ease-in-out hover:text-primary dark:text-primary dark:hover:text-slate-300"
                            ...
                            <div class="flex items-center justify-center pt-5 md:justify-start">
                                {{-- Youtube icon start --}}
                                <a class="mr-3 flex h-9 w-9 items-center justify-center rounded-full border border-slate-300 transition duration-300 ease-in-out hover:border-primary hover:bg-primary hover:text-white dark:text-white"
                    ...
                    <div class="border-lila border-t py-5 dark:border-slate-500">
                        <h3 class="mb-3 mt-1 text-center text-lg font-bold uppercase text-primary">
                            Artikel Terkait
                        </h3>
                        <div class="border-lila flex justify-center border-t py-5 dark:border-slate-500">
                            <div class="flex flex-wrap">
                                ...
                                @empty
                                    <div class="w-full px-4">
                                        <p class="text-base font-medium text-secondary dark:text-slate-400 sm:text-center">
                                            Belum ada artikel terkait dengan postingan
                                            <span class="text-primary">
                                                {{ $post->title }}
                                            </span>
                                            <br> <br>
                                            <span class="text-lg font-bold">
                                                Yuk cek postingan lainnya melalui tombol link dibawah ini
                                            </span>
                                        </p>
